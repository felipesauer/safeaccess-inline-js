 ...
 * {@link CsvParser} with no external dependencies.
 *
 * @api
 *
 * @example
 * const accessor = new CsvAccessor(parser).from('name,age\nAlice,30');
    ...
    from(data: unknown): this {
        if (typeof data !== 'string') {
            ...
            throw new InvalidFormatException(
                `CsvAccessor expects a CSV string, got ${typeof data}`,
            );
